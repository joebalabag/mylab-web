  ...
  <div class="flex h-full flex-col gap-4">
    <div class="card flex flex-1 min-h-0 flex-col overflow-hidden">
      <div class="card-header">
        <div>
          <div class="text-sm font-semibold text-slate-800">Users</div>
    ...
    <Modal :show="showForm" :title="editing ? 'Edit User' : 'Add User'" size="lg" @close="showForm = false">
      <form id="userForm" @submit.prevent="submit" class="space-y-5">
        ...
        <fieldset class="space-y-3">
          <legend class="mb-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">Identity</legend>
          <div>
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div>
              <label class="label">Email <span class="text-slate-400">(optional)</span></label>
        ...
        <fieldset class="space-y-3 rounded-lg border border-slate-200 bg-slate-50/60 p-4">
          <legend class="px-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">Login credentials</legend>
          <div>
            ...
            <input v-model="form.username" :disabled="!!editing" required minlength="3" maxlength="100"
                   autocomplete="off" class="input font-mono lowercase"
          ...
          <div v-else class="rounded-md border border-slate-200 bg-white px-3 py-2 text-[11px] text-slate-600">
        ...
        <fieldset class="space-y-3 rounded-lg border border-slate-200 p-4">
          <legend class="px-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">
            Lab report fields <span class="ml-1 text-slate-400 normal-case tracking-normal">(optional)</span>
          </legend>
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              ...
              <p class="mt-1 text-[11px] text-slate-500">Printed under the signature.</p>
            </div>
            <div>
              ...
              <p class="mt-1 text-[11px] text-slate-500">Falls back to Full name when blank.</p>
            </div>
          </div>
        </fieldset>
      </form>
        ...
        <div class="max-h-[55vh] space-y-3 overflow-y-auto pr-1">
          <div v-for="group in accessTree" :key="group.key"
            ...
            <div class="flex items-center justify-between border-b border-slate-100 px-3 py-2">
              <div class="min-w-0">
                <div class="text-sm font-semibold text-slate-800 capitalize">{{ group.label }}</div>
              </div>
            ...
            <div class="divide-y divide-slate-100">
              <div v-for="mod in group.modules" :key="mod.key" class="p-3"
                ...
                <label v-if="!Array.isArray(mod.actions)"
                       class="flex items-start gap-2 rounded-md px-2 py-1 text-sm text-slate-700"
                       :class="isModulePlanLocked(mod) ? 'cursor-not-allowed' : 'cursor-pointer hover:bg-slate-50'">
                  <input type="checkbox"
                  ...
                  <span class="min-w-0 flex-1">
                    <span class="block font-medium capitalize leading-tight text-slate-800">
                      {{ mod.label }}
