    ...
    <div class="print-area space-y-4">
      ...
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <div class="card"><div class="card-body">
          <div class="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 dark:text-slate-500">Total Spent</div>
          <div class="mt-1 text-2xl font-bold text-rose-600 tabular-nums">{{ money(data.grand_total || 0) }}</div>
          <div class="text-[11px] text-slate-500 dark:text-slate-400 dark:text-slate-500">{{ data.count || 0 }} entries · active only</div>
        </div></div>
        <div class="card"><div class="card-body">
          ...
          <div class="mt-1 text-2xl font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 tabular-nums line-through">{{ money(data.voided_total || 0) }}</div>
        </div></div>
        <div class="card"><div class="card-body">
          ...
          <div class="mt-1 text-2xl font-bold text-slate-800 dark:text-slate-100">{{ data.by_category?.length || 0 }}</div>
        </div></div>
      </div>
      ...
      <div class="grid grid-cols-1 gap-3 lg:grid-cols-3">
        <div class="card">
          <div class="card-header">
            <div class="text-sm font-semibold text-slate-800 dark:text-slate-100">Category Share</div>
          </div>
          <div class="card-body">
            <div v-if="loading" class="h-56 animate-pulse rounded bg-slate-50 dark:bg-slate-800"></div>
            <div v-else-if="!catLabels.length" class="flex h-56 items-center justify-center text-xs text-slate-400 dark:text-slate-500">
        ...
        <div class="card lg:col-span-2 overflow-hidden">
          <div class="card-header">
            <div class="text-sm font-semibold text-slate-800 dark:text-slate-100">Totals by Category</div>
          </div>
            ...
            <table class="w-full text-xs">
              ...
              <tbody>
                <tr v-if="!data.by_category?.length && !loading">
                  <td colspan="3" class="py-4 text-center text-xs text-slate-400 dark:text-slate-500">No expenses.</td>
                </tr>
                <tr v-for="c in data.by_category" :key="c.category" class="border-b border-slate-100 dark:border-slate-800">
              ...
              <tfoot v-if="data.by_category?.length" class="bg-slate-100 dark:bg-slate-800">
                <tr class="font-bold">
      ...
      <div class="card overflow-hidden">
        <div class="card-header">
          <div class="text-sm font-semibold text-slate-800 dark:text-slate-100">Expense Entries</div>
          <span class="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Sorted by date, newest first</span>
        </div>
        <div class="overflow-x-auto">
          <table class="table w-full text-xs">
            ...
            <tbody>
              <tr v-if="!data.rows?.length && !loading">
                <td colspan="6" class="py-4 text-center text-xs text-slate-400 dark:text-slate-500">No expenses in this range.</td>
              </tr>
              <tr v-for="r in data.rows" :key="r.uuid"
                  class="border-b border-slate-100 dark:border-slate-800"
                  :class="isVoid(r) && 'bg-slate-50/50 dark:bg-slate-800/50'">
                <td>{{ formatDate(r.date_transact) }}</td>
                <td>{{ r.category }}</td>
                <td class="max-w-md truncate">
                  {{ r.description }}
                  <div v-if="r.notes" class="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-500 italic truncate">{{ r.notes }}</div>
                </td>
                <td class="text-right font-semibold tabular-nums"
                    :class="isVoid(r) ? 'text-slate-400 dark:text-slate-500 line-through' : 'text-rose-700'">
                  {{ money(r.amount) }}
                </td>
                <td class="hidden sm:table-cell text-slate-600 dark:text-slate-300">{{ r.cashier_name || r.created_by || '—' }}</td>
