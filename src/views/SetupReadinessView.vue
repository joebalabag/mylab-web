  ...
  <div class="flex h-full flex-col gap-4 overflow-auto pb-8">
    ...
    <div class="card">
      <div class="card-body">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div class="text-sm font-semibold text-slate-800">Setup Readiness</div>
        ...
        <div v-if="data" class="mt-4 space-y-2">
          <div class="flex flex-wrap items-baseline justify-between gap-2 text-xs">
            <div>
              ...
              <span class="text-slate-500"> of {{ data.required_total }} required steps done</span>
              <span class="ml-2 text-slate-400">· {{ data.completed }} of {{ data.total }} total (with optional)</span>
            </div>
    ...
    <div v-for="phase in phases" :key="phase.key" class="card">
      <div class="card-header flex items-baseline justify-between">
        <div>
          <div class="text-sm font-semibold text-slate-800">{{ phase.label }}</div>
      ...
      <div class="card-body p-0">
        <div v-if="loading && !data" class="p-6 text-center text-xs text-slate-400">Loading…</div>
        <ul v-else class="divide-y divide-slate-100">
          <li v-for="step in phase.steps" :key="step.key"
              class="flex items-start gap-3 p-3 hover:bg-slate-50/60">
            <!-- Status circle -->
            ...
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                <div class="text-sm font-semibold text-slate-800">{{ step.label }}</div>
                <span v-if="!step.required"
                      class="rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-slate-500">
                  Optional
                </span>
                <span v-if="step.done" class="text-[11px] font-semibold text-emerald-700">
                  {{ step.detail }}
                </span>
                <span v-else-if="step.detail" class="text-[11px] text-slate-500">
                  {{ step.detail }}
                </span>
              </div>
              <div class="text-xs text-slate-600">{{ step.description }}</div>
            </div>

            <router-link :to="step.route"
                         class="btn-secondary !text-xs shrink-0"
                         :class="step.done && '!bg-white !text-slate-500'">
