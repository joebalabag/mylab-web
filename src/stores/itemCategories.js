...
import * as api from '../api/itemCategories'
...
export const useItemCategoriesStore = defineStore('itemCategories', {
  ...
  getters: {
    activeItems: (s) => s.items.filter(c => c.status === 'active'),
  ...
  actions: {
    async fetch(overrides = {}) {
      ...
      try {
        ...
        const res = await api.listItemCategories({
        ...
        this.error = e?.message || 'Failed to load item categories'
        throw e
      } finally {
        this.loading = false
      }
    ...
    async update(uuid, payload) {
      const updated = await api.updateItemCategory(uuid, payload)
      if (updated?.uuid) {
    ...
    async remove(uuid) {
      await api.deleteItemCategory(uuid)
    ...
    async setStatus(uuid, status) {
      const updated = await api.setItemCategoryStatus(uuid, status)
      if (updated?.uuid) {
        const idx = this.items.findIndex(c => c.uuid === uuid)
        if (idx >= 0) this.items[idx] = { ...this.items[idx], ...updated }
      }
