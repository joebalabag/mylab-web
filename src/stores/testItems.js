...
import * as api from '../api/testItems'
...
export const useTestItemsStore = defineStore('testItems', {
  state: () => ({
    items: [],
    total: 0,
    pageNumber: 0,
  ...
  getters: {
    activeItems: (s) => s.items.filter(t => t.status === 'active'),
    byUuid:      (s) => (uuid) => s.items.find(t => t.uuid === uuid) || null
  },
  actions: {
    async fetch(overrides = {}) {
      ...
      try {
        ...
        const res = await api.listTestItems({
          ...
          result_type:        merged.result_type        || undefined,
        ...
        this.error = e?.message || 'Failed to load test items'
        throw e
      } finally {
        this.loading = false
      }
    ...
    async create(payload) {
      const created = await api.createTestItem(payload)
      if (created?.uuid) {
    ...
    async update(uuid, payload) {
      const updated = await api.updateTestItem(uuid, payload)
      if (updated?.uuid) {
        const idx = this.items.findIndex(t => t.uuid === uuid)
    ...
    async setStatus(uuid, status) {
      const updated = await api.setTestItemStatus(uuid, status)
      if (updated?.uuid) {
        const idx = this.items.findIndex(t => t.uuid === uuid)
        if (idx >= 0) this.items[idx] = { ...this.items[idx], ...updated }
      }
