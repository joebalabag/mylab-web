  ...
  <Teleport to="body">
    <transition name="fade">
      <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6">
        ...
        <div class="relative flex h-full w-full max-w-5xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
          <!-- Header -->
          <div class="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 sm:px-6">
            ...
            <div class="min-w-0 flex-1">
              <div class="text-sm font-bold text-slate-800">Getting Started</div>
              <div class="text-xs text-slate-500">MyLab · v{{ appVersion }}</div>
            </div>
          ...
          <nav class="shrink-0 overflow-x-auto border-b border-slate-100 bg-slate-50 px-3 py-2 lg:hidden">
            <ul class="flex items-center gap-1.5 whitespace-nowrap">
              <li v-for="s in sections" :key="s.key">
                <button type="button"
                        class="rounded-full bg-white px-3 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200 hover:bg-slate-100 hover:text-slate-900"
          ...
          <div class="flex min-h-0 flex-1 overflow-hidden">
            ...
            <aside class="hidden w-56 shrink-0 overflow-y-auto border-r border-slate-100 bg-slate-50 p-3 lg:block">
              <div class="mb-2 px-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">Contents</div>
              <ul class="space-y-0.5 text-sm">
                <li v-for="s in sections" :key="s.key">
                  <button type="button"
                          class="w-full rounded-md px-2 py-1 text-left text-slate-600 hover:bg-white hover:text-slate-900"
                          @click="scrollTo(s.key)">
                    {{ s.label }}
                  </button>
                </li>
            ...
            <main ref="scrollRoot" class="min-w-0 flex-1 space-y-6 overflow-y-auto bg-slate-50 p-4 sm:p-6">
              <!-- Hero -->
              <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h1 class="text-2xl font-bold text-slate-800">Welcome to MyLab 🧪</h1>
              ...
              <section id="welcome" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-bold text-slate-800">What you can do with MyLab</h2>
                <div class="mt-3 grid gap-3 sm:grid-cols-3">
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">Manage patients &amp; cases</div>
                    <p class="mt-1 text-xs text-slate-600">Patient master records, per-visit cases, and per-case requisitions for the tests you'll run.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">Bill &amp; collect at the counter</div>
                    <p class="mt-1 text-xs text-slate-600">Cashier settles unpaid items. Cash, e-wallet, bank transfer, or an arrangement (A/R, insurance, paid outside, charity).</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">Sign &amp; release lab reports</div>
                    <p class="mt-1 text-xs text-slate-600">Encode results (single, panel, matrix, narrative), tag as final with medtech + pathologist signatures, print / share via QR.</p>
                  </div>
                </div>
              ...
              <!-- ─── 1 · Register ─────────────────────────────────────── -->
              <section id="signup" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">1</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">Register your laboratory</h2>
                    <p class="mt-1 text-sm text-slate-600">Create your tenant workspace.</p>
                  </div>
                </div>
                <div class="mt-4 space-y-3">
                  ...
                  <ul class="space-y-1 text-sm text-slate-700">
                    <li class="flex items-start gap-2"><span class="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400"></span><span><b>Laboratory name</b> — printed on receipts and lab reports.</span></li>
                    <li class="flex items-start gap-2"><span class="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400"></span><span><b>Contact email</b> — receives the verification link + your admin login credentials.</span></li>
                    <li class="flex items-start gap-2"><span class="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400"></span><span><b>Owner name</b>, <b>Contact number</b>, <b>City / Province / Country</b>.</span></li>
                  </ul>
              ...
              <!-- ─── 2 · Confirm email ────────────────────────────────── -->
              <section id="email" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">2</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">Confirm your email</h2>
                    <p class="mt-1 text-sm text-slate-600">One click activates your trial workspace.</p>
                  </div>
                </div>
              ...
              <!-- ─── 3 · First sign-in ────────────────────────────────── -->
              <section id="signin" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">3</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">First sign-in</h2>
                    <p class="mt-1 text-sm text-slate-600">Get inside and secure your account.</p>
                  </div>
                </div>
                <ol class="mt-4 list-inside list-decimal space-y-1 text-sm text-slate-700">
              ...
              <!-- ─── 4 · Company & branding ──────────────────────────── -->
              <section id="company" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">4</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">Company &amp; branding</h2>
                    <p class="mt-1 text-sm text-slate-600">Set the identity used on every printed receipt and lab report.</p>
                  </div>
                </div>
                <p class="mt-4 text-sm text-slate-700">Open <b>Company Settings</b> from the sidebar and fill in:</p>
                <div class="mt-3 grid gap-3 sm:grid-cols-2">
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Company Identity</div>
                    <p class="mt-1">Display name, legal name, owner, branch, currency. Appears at the top of receipts and lab reports.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Company Logo</div>
                    <p class="mt-1">JPG / PNG / WebP / SVG up to 5 MB. Renders on the sidebar and lab report header (logo-mode).</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Address, Contact, TIN</div>
                    <p class="mt-1">Contact info + tax ID printed on receipts. Toggle VAT-registered on/off.</p>
                  </div>
                  <div class="rounded-lg border border-brand-100 bg-brand-50/40 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Lab Report Result Header</div>
                  ...
                  <div class="rounded-lg border border-emerald-100 bg-emerald-50/40 p-3 text-xs text-slate-700 sm:col-span-2">
                    <div class="font-semibold text-slate-800">Cashier Receipt Header</div>
              ...
              <!-- ─── 5 · Signatory doctors ───────────────────────────── -->
              <section id="doctors" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">5</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">Add signatory doctors</h2>
                    <p class="mt-1 text-sm text-slate-600">Pathologists and radiologists who sign the finalized reports.</p>
                  </div>
                </div>
                ...
                <ul class="mt-3 space-y-1 text-sm text-slate-700">
                  <li class="flex items-start gap-2"><span class="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400"></span><span><b>Name</b> — printed under the signature line.</span></li>
                  <li class="flex items-start gap-2"><span class="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400"></span><span><b>Specialty</b> — Pathologist, Radiologist, etc.</span></li>
                  <li class="flex items-start gap-2"><span class="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400"></span><span><b>License #</b> — printed as "Lic. No. …" under the signature.</span></li>
                  <li class="flex items-start gap-2"><span class="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400"></span><span><b>E-signature</b> (optional) — upload a transparent-background PNG. Overlays the printed name on final reports.</span></li>
                </ul>
                <div class="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
              ...
              <!-- ─── 6 · Test catalog ────────────────────────────────── -->
              <section id="catalog" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">6</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">Build your test catalog</h2>
                    <p class="mt-1 text-sm text-slate-600">The billable menu — item groups → categories → test items.</p>
                  </div>
                </div>
                ...
                <p class="mt-4 text-sm text-slate-700">Or add each level yourself:</p>
              ...
              <section id="discounts" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">7</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">Discounts &amp; item packages <span class="text-sm font-normal text-slate-500">(optional)</span></h2>
                    <p class="mt-1 text-sm text-slate-600">Common promos and bundled panels the cashier can apply.</p>
                  </div>
                </div>
                <div class="mt-3 grid gap-3 sm:grid-cols-2">
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Discounts</div>
                  ...
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Item Packages</div>
              ...
              <!-- ─── 8 · Team ─────────────────────────────────────────── -->
              <section id="team" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">8</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">Add your team</h2>
                    <p class="mt-1 text-sm text-slate-600">Give medtechs, cashiers, and managers their own logins.</p>
                  </div>
                </div>
                <ol class="mt-4 list-inside list-decimal space-y-1 text-sm text-slate-700">
                ...
                <div class="mt-4 grid gap-3 sm:grid-cols-3">
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs">
                    <div class="text-sm font-semibold text-slate-800">Medtech / Encoder</div>
                    <p class="mt-1 text-slate-600">Patients, Cases, Laboratory (add / edit / tag as final).</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs">
                    <div class="text-sm font-semibold text-slate-800">Cashier</div>
                    <p class="mt-1 text-slate-600">Cashier (dashboard + new payment + resolve arrangement). No void — that's manager-only.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs">
                    <div class="text-sm font-semibold text-slate-800">Manager / Admin</div>
                    <p class="mt-1 text-slate-600">Everything — including void, discount overrides, and User Management.</p>
                  </div>
                </div>
              ...
              <section id="operations" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-emerald-100 font-bold text-emerald-700">9</div>
                  <div class="flex-1">
                    <h2 class="text-lg font-bold text-slate-800">Everyday operations 🩺</h2>
                    <p class="mt-1 text-sm text-slate-600">The end-to-end flow every patient goes through.</p>
                  </div>
                </div>
                ...
                <div class="mt-4 space-y-3">
                  <div class="rounded-lg border border-slate-200 bg-white p-3">
                    <div class="flex items-center gap-2">
                      ...
                      <div class="text-sm font-semibold text-slate-800">Add a case</div>
                    </div>
                    <div class="mt-1 pl-8 text-xs text-slate-600 space-y-1.5">
                      <p>
                      ...
                      <div class="mt-2 rounded-md border border-slate-200 bg-slate-50 p-2 text-[11px] text-slate-600">
                  ...
                  <div class="rounded-lg border border-slate-200 bg-white p-3">
                    <div class="flex items-center gap-2">
                      ...
                      <div class="text-sm font-semibold text-slate-800">Add tests, finalize the requisition</div>
                    </div>
                    <p class="mt-1 pl-8 text-xs text-slate-600">
                  ...
                  <div class="rounded-lg border border-slate-200 bg-white p-3">
                    <div class="flex items-center gap-2">
                      ...
                      <div class="text-sm font-semibold text-slate-800">Cashier collects payment</div>
                    </div>
                    <p class="mt-1 pl-8 text-xs text-slate-600">
                      <b>Cashier → + New Payment</b>. Search the case, tick which items to bill, apply a discount if needed, pick the method:
                    </p>
                    <ul class="mt-1 pl-8 space-y-0.5 text-xs text-slate-600">
                    ...
                    <p class="mt-1 pl-8 text-xs text-slate-600">Receipt opens for print after confirming. Void requires manager re-authentication.</p>
                  </div>
                  <div class="rounded-lg border border-slate-200 bg-white p-3">
                    <div class="flex items-center gap-2">
                      ...
                      <div class="text-sm font-semibold text-slate-800">Encode results in Laboratory</div>
                    </div>
                    <p class="mt-1 pl-8 text-xs text-slate-600">
                  ...
                  <div class="rounded-lg border border-slate-200 bg-white p-3">
                    <div class="flex items-center gap-2">
                      ...
                      <div class="text-sm font-semibold text-slate-800">Tag as Final &amp; print</div>
                    </div>
                    <p class="mt-1 pl-8 text-xs text-slate-600">
              ...
              <!-- ─── Dashboard ───────────────────────────────────────── -->
              <section id="dashboard" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-3 grid gap-3 sm:grid-cols-2">
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Revenue KPIs</div>
                    <p class="mt-1">Collected revenue, outstanding A/R, today's cash, revenue trend line, payment-method doughnut.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Receivables aging</div>
                  ...
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Throughput &amp; top tests</div>
                    <p class="mt-1">Cases and requisitions per day + the 10 highest-revenue tests for the period.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Subscription card</div>
              ...
              <!-- ─── Setup Readiness ─────────────────────────────────── -->
              <section id="readiness" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-bold text-slate-800">Setup Readiness</h2>
                <p class="mt-2 text-sm text-slate-700">
              ...
              <!-- ─── Reports ─────────────────────────────────────────── -->
              <section id="reports" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-bold text-slate-800">Reports</h2>
                <p class="mt-2 text-sm text-slate-700">
                ...
                <div class="mt-3 grid gap-3 sm:grid-cols-2">
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Summary</div><p class="mt-1 text-slate-600">Overall snapshot + transaction analytics: revenue, ticket sizes, method mix, top cashiers, daily trend.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Monthly Sales</div><p class="mt-1 text-slate-600">Jan-Dec breakdown for the selected year — revenue, discounts, expenses, net.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Monthly Tests</div><p class="mt-1 text-slate-600">Lab reports created per month, with finalized/voided counts and percentages.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Cashier Sales</div><p class="mt-1 text-slate-600">Per-cashier revenue for the range. Filter by cashier name.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Voids</div><p class="mt-1 text-slate-600">Every voided payment — created by, voided by, when, why.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Daily Sales</div><p class="mt-1 text-slate-600">Day-by-day sales tape with cash-only subtotal for reconciliation.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Daily Detailed Sales</div><p class="mt-1 text-slate-600">Every payment, itemized by test, grouped by day. Great for audit.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Daily Tests</div><p class="mt-1 text-slate-600">Lab reports created per day with finalized/voided counts + percentages.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Discounts</div><p class="mt-1 text-slate-600">Which discount codes fired, how often, and how much was given away.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Payment Summary</div><p class="mt-1 text-slate-600">Method × status breakdown with a doughnut chart.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Expense Report</div><p class="mt-1 text-slate-600">All expenses (or filtered by category) with category share doughnut + totals table.</p></div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs"><div class="font-semibold text-slate-800">Test Analytics</div><p class="mt-1 text-slate-600">Test volume trend, top-20 tests, category mix, and turnaround time (median / p95) per category.</p></div>
                </div>
              </section>

              <!-- ─── Subscription ────────────────────────────────────── -->
              <section id="renew" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-bold text-slate-800">Subscription &amp; renewal</h2>
                <p class="mt-2 text-sm text-slate-700">
                ...
                <ol class="mt-3 list-inside list-decimal space-y-1 text-sm text-slate-700">
              ...
              <!-- ─── Tips ────────────────────────────────────────────── -->
              <section id="tips" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-bold text-slate-800">Everyday tips</h2>
                <div class="mt-3 grid gap-3 sm:grid-cols-2">
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Check the Dashboard first thing</div>
                    <p class="mt-1">Yesterday's revenue, today's cash, oldest pending A/R — all in one glance.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Chase A/R weekly</div>
                    <p class="mt-1">Open the Arrangements tab in Cashier; the oldest-first list is your call-list. Resolve when settled.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Print Preview before printing for real</div>
                  ...
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700">
                    <div class="font-semibold text-slate-800">Rotate passwords when staff turnover happens</div>
              ...
              <section id="faq" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-bold text-slate-800">Common questions</h2>
                <div class="mt-4 space-y-3">
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">A menu item is missing from my sidebar.</div>
                    <p class="mt-1 text-xs text-slate-600">Ask an admin or manager to open <b>User Management → Assign access</b> and tick the module for you.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">I typed a URL and got sent back to the Dashboard.</div>
                    <p class="mt-1 text-xs text-slate-600">You don't have the access row for that page. This is a security feature — request it from a manager.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">The app is locked with "Subscription expired".</div>
                    <p class="mt-1 text-xs text-slate-600">Open <b>Subscription</b> and submit a renewal payment. Access returns as soon as our team confirms.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">The cashier can't find a case with unpaid items.</div>
                    <p class="mt-1 text-xs text-slate-600">The requisition may still be in <b>draft</b>. Open the case → the draft requisition → hit <b>Finalize</b>. Its items become billable immediately.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">Print preview came out blank.</div>
                    <p class="mt-1 text-xs text-slate-600">The print popup was firing before the tenant logo / QR code finished loading. This is fixed in the current build — assets are awaited explicitly before the print dialog opens.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">A test item is missing from the requisition picker.</div>
                    <p class="mt-1 text-xs text-slate-600">Check that the test item is <b>active</b> and belongs to an active category + group. Deactivated items don't show up in pickers.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">Void payment says "manager approval required".</div>
                    <p class="mt-1 text-xs text-slate-600">Cashiers can't self-void. The modal asks for a manager/admin username + password to sign off; the void is logged with that person's name in the Transact By column.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">The pathologist's name isn't printing on final reports.</div>
                    <p class="mt-1 text-xs text-slate-600">Open the parent <b>Item Group</b> → assign a <b>Signatory Doctor</b>. That doctor's name / license / e-signature will auto-fill on every finalized report under that group.</p>
                  </div>
                  <div class="rounded-lg border border-slate-100 bg-slate-50 p-3">
                    <div class="text-sm font-semibold text-slate-800">I imported the pre-loaded catalog twice — will I get duplicates?</div>
                    <p class="mt-1 text-xs text-slate-600">No. The import is idempotent: it checks each code and skips whatever already exists. Safe to re-run any time.</p>
                  </div>
                </div>
              </section>

              <div class="pb-4 text-center text-[11px] text-slate-400">
